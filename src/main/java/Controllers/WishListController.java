...
public class WishListController extends HttpServlet {
    ...
    protected void doGet(jakarta.servlet.http.HttpServletRequest request, jakarta.servlet.http.HttpServletResponse response)
        ...
        if (path.startsWith("/wishlist/delete/")) {
            try {
                WishListDAO wishDAO = new WishListDAO();
                String[] data = path.split("/");
                int wishListId = Integer.parseInt(data[data.length - 1]); //lay id dua tren duong dan de xoa
                int kq = wishDAO.Delete(wishListId); //thuc hien chuc nang xoa
                if (kq != 0) {//xoa thanh cong
        ...
        } else if (path.endsWith("/wishlist/show")) {
            try {
